...
def find_sibling_text(soup, child: str, sibling: str, contains_string: str, sibling_type="prev|next"):
...
def extract_hyper_link(soup_chunk, patterns: list, **kwargs):
    ...
    if with_text:
        if prefix:
            urls_found = [{"text": get_clean_text(link.text), "link": "{}{}".format(prefix, link["href"])} for link
                          in soup_chunk.find_all("a", href=True) if len(validate(link["href"])) > 0]
        else:
            urls_found = [{"text": get_clean_text(link.text), "link": link["href"]} for link in
                          soup_chunk.find_all("a", href=True) if len(validate(link["href"])) > 0]
    else:
        if prefix:
            urls_found = ["{}{}".format(prefix, link["href"]) for link in soup_chunk.find_all("a", href=True) if
                          len(validate(link["href"])) > 0]
        else:
            urls_found = [link["href"] for link in soup_chunk.find_all("a", href=True) if
                          len(validate(link["href"])) > 0]
    return urls_found
...
def extract_image_link(soup_chunk, **kwargs):
    ...
    validate = lambda url: [1 for i in patterns if str(url).lower().__contains__(str(i).lower())]
    if prefix:
        urls_found = ["{}{}".format(prefix, link["src"]) for link in soup_chunk.find_all("img", src=True) if
                      len(validate(link["src"])) > 0]
    else:
        urls_found = [link["src"] for link in soup_chunk.find_all("img", src=True) if
                      len(validate(link["src"])) > 0]
    return urls_found
